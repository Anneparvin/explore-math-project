...
function isJavaScriptFile(File,target){
    return File.endsWith(target);
}
// console.log(isJavaScriptFile("image.png.js",".js"));
...
function oilPrice(orderDizel,orderPetrol,orderOcten) {
    const perLitreDizel = 114;
    const perLitrePetrol = 130;
    const perLitreOcten = 135;
    const totalPerLitre = (perLitreDizel * orderDizel) + (perLitrePetrol * orderPetrol) + (perLitreOcten * orderOcten);
    return totalPerLitre;
}
...
function isBestFriend(friends,anotherFriends){
    if (friends.name == anotherFriends.friend && anotherFriends.name == friends.friend){
        return true;
    }
else{
    return false;
...
const friends = [{name: "abul", friend:"babul"}];
const anotherFriends =[{name: "babul", friend:"abul"}];
let bestFriend = isBestFriend(friends,anotherFriends);
...
function publicBusFare (countPeople){
    let busPeople = countPeople % 50;
    let microBusPeople = busPeople % 11;
    let publicBusPeople = microBusPeople * 250;
     return publicBusPeople;
}
